...
- **Mint short-lived tokens** — the gateway intentionally does **not** revoke JWTs on logout (revocation is the wrapping backend's job, per the threat model in §"Gateway Must Be Wrapped"). A token stays valid until natural expiry, so the backend should mint short-lived tokens (or rotate the token on logout) for sensitive deployments. `JWT_TOKEN_DURATION_MINUTES` is clamped to 1..525600 (24h..1y); out-of-range values reset to the 24h default.
