...
If you provided an `hmac_secret`, webhook requests will include an `X-Webhook-Signature` header of the form `sha256=<hex_signature>`, where the signature is the HMAC-SHA256 of the **raw request body** (before any JSON parsing). Verify it like this:

> ⚠️ The header is `X-Webhook-Signature` (not `X-Signature`), and it includes the `sha256=` prefix — strip it before comparing. Compute the HMAC over the **raw bytes**, not a re-serialized object.
...
function verifyWebhook(rawBody, signatureHeader, secret) {
  ...
  const expected = crypto
    ...
    .update(rawBody) // raw request body bytes / Buffer
    .digest('hex');
...
func verifyWebhook(rawBody []byte, signatureHeader, secret string) bool {
	...
	mac.Write(rawBody) // raw request body
...
- `"lid"` — the only available identifier was an opaque `@lid` (typically a group participant whose phone number couldn't be resolved). Do **not** assume `from` is dialable in this case; treat it as an opaque sender key.
